...
def apply_manual_attacks(pipeline):
    ...
    attack_level = getattr(pipeline.cfg.algo.safe_controller, 'attack_level', 'medium')

    try:
        safety_index = pipeline.algo.safe_controller.safety_index
        print("Found safety_index via pipeline.algo.safe_controller.safety_index")
    except:
        print("Could not access safety_index. Skipping attack injection.")
        return

    if attack_type == "perception_noise":
        std = {"low": 0.02, "medium": 0.05, "high": 0.10}.get(attack_level, 0.05)
        
        original_compute = getattr(safety_index, 'compute', None)
        
        if original_compute:
            def noisy_compute(self, *args, **kwargs):
                result = original_compute(*args, **kwargs)
                print(f"Perception noise applied (std={std}) - (basic patch)")
                return result
            
            safety_index.compute = noisy_compute.__get__(safety_index)
            print(f"Perception noise injected (std={std})")
        else:
            print("Could not find compute method to patch for noise")

    elif attack_type == "latency":
        delay = {"low": 2, "medium": 5, "high": 10}.get(attack_level, 5)
        ...
        if original_compute:
            def delayed_compute(self, *args, **kwargs):
                current = original_compute(*args, **kwargs)
                safety_index.latency_buffer.append(current)
                if len(safety_index.latency_buffer) > safety_index.latency_delay:
                    return safety_index.latency_buffer.pop(0)
                return current
            
            safety_index.compute = delayed_compute.__get__(safety_index)
            print(f"Latency attack injected (delay={delay} steps)")
        else:
            print("Could not find compute method to patch for latency")
